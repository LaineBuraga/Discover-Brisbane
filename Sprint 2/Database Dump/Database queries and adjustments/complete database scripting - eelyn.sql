...
/*change client_type to enum values
#update client set client_type = 'Admin' where username = 'Susan';
#alter table client modify client_type ENUM('Tourist', 'Student', 'Business', 'Admin') NOT NULL;
select * from client;
...
SELECT * FROM ifb299.location;
/*
update location
set photo_name = 'UQ_Bris.jpg'
where location_id=2;

update location
set photo_name = 'Brisbane Square.jpg'
where location_id=4;*/

#alter table feedback drop primary key;
#alter table feedback add column `name` VARCHAR(45) NOT NULL;
alter table feedback add column feedback_id INT auto_increment PRIMARY KEY;
...
#Add new columns and reassign primary key
alter table feedback add column `name` VARCHAR(45) NOT NULL;
alter table feedback add column feedback_id INT auto_increment PRIMARY KEY;
